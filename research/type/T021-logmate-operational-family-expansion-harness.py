"""T021 LogMate operational-family expansion harness — structural revision.

Research-only bounded font. Kerning stays OFF. The harness validates declared
repertoire, explicit construction, cmap coverage and 14/17/24 px advances.
It intentionally raises rather than silently falling back to placeholder boxes.
"""
...
AMBIG=['0O','1Il','5S','8B']; SPACING=['HHOO','HOHOHO','nono','noon','AVAVA','TOTO','LITIL']
...
def ring(p,x0,y0,x1,y1,t=STEM):
 # Clockwise outer + counter in reverse winding. Polygonal research primitive,
 # but structurally valid and deterministic; no additive pseudo-erasure.
...
def open_round(p,x0,y0,x1,y1,t=STEM,kind='C'):
 # Explicit open contour built from three strokes; right side is genuinely open.
...
def glyph_upper(c,aw,lsb):
 ...
 if c=='O': ring(p,lsb,-10,r,CAP+10)
  ...
  # Deliberately angular but structurally S-shaped: top/right, middle, bottom/left.
  rect(p,lsb,615,r,CAP); rect(p,lsb,310,r,390); rect(p,lsb,0,r,STEM); rect(p,lsb,390,lsb+STEM,CAP); rect(p,r-STEM,0,r,310)
  ...
  rect(p,lsb,0,lsb+STEM,CAP); rect(p,r-STEM,0,r,CAP); poly(p,[(lsb+STEM,CAP),(lsb+2*STEM,CAP),(m,365),(m,245)]); poly(p,[(r-2*STEM,CAP),(r-STEM,CAP),(m,365),(m,245)])
...
def glyph_lower(c):
 ...
 if c=='l': rect(p,170,0,170+STEM,CAP); rect(p,145,0,320,STEM)
  ...
  rect(p,l,0,l+STEM,XH); rect(p,r-STEM,0,r,365); rect(p,l+STEM,418,r-STEM,XH); rect(p,r-STEM,330,r,XH)
...
def build():
 ...
 assert set(UPPER)==set(widths), (set(UPPER)-set(widths), set(widths)-set(UPPER))
 for c in UPPER:
 ...
 p=TTGlyphPen(None); # direct E + acute outline; no mark/GPOS claim
 rect(p,60,0,142,CAP); rect(p,60,615,570,CAP); rect(p,60,310,520,390); rect(p,60,0,570,STEM); poly(p,[(300,760),(390,850),(475,850),(365,760)])
 glyphs['Eacute']=p.glyph(); metrics['Eacute']=(570,60); cmap[0x00C9]='Eacute'; order.append('Eacute')
 fb=FontBuilder(UPM,isTTF=True); fb.setupGlyphOrder(order); fb.setupCharacterMap(cmap); fb.setupGlyf(glyphs); fb.setupHorizontalMetrics(metrics); fb.setupHorizontalHeader(ascent=900,descent=-200); fb.setupOS2(sTypoAscender=900,sTypoDescender=-200,usWinAscent=900,usWinDescent=200); fb.setupNameTable({'familyName':'T021 LogMate Balanced Expanded','styleName':'Regular','uniqueFontIdentifier':'T021-LogMate-Balanced-Expanded-R2','fullName':'T021 LogMate Balanced Expanded R2','psName':'T021-LogMate-Balanced-Expanded-R2'}); fb.setupPost(); fb.setupMaxp()
 path=OUT/'T021-LogMate-Balanced-Expanded-R2.ttf'; fb.save(path); return path,cmap

path,cmap=build(); required=set(''.join(STRINGS)); missing=sorted(ch for ch in required if ord(ch) not in cmap)
res={'revision':'R2-structural','upm':UPM,'kerning':False,'artifact':path.name,'declared_uppercase':UPPER,'required_unique_nonspace':len(required-{' '}),'covered_unique_nonspace':len((required-{' '})-set(missing)),'missing':missing,'sizes_px':[14,17,24],'measurements':{}}
for px in (14,17,24):
 f=ImageFont.truetype(str(path),px); res['measurements'][str(px)]={s:round(f.getlength(s),4) for s in STRINGS}
(OUT/'T021-logmate-operational-family-expansion-R2-results.json').write_text(json.dumps(res,indent=2),encoding='utf-8')
